...
//!
//! Ten of those definitions ship with rulogman and are compiled into the binary:
//! C, C++, C#, Go, Java, JavaScript, Python, Rust, SQL and TypeScript. They are
//! ordinary definition files that happen to live in `crates/rulogman-app/syntaxes`
//! — [`SHIPPED`] — so there is nothing they can do that a file of the user's
//! cannot.
//!
//! # Where the files go, and when they are read
//!
//! One `*.yml` (or `*.yaml`) file per language in [`rulogman_core::syntaxes_dir`]
//! — `~/.config/rulogman/syntaxes/` on Linux, `syntaxes` beside `settings.json`
...
/// The definitions rulogman ships, by id, compiled into the binary.
///
/// They live in `crates/rulogman-app/syntaxes/` — beside `locales/`, and in the
/// same spirit: a language rulogman knows is part of the build, not an asset to
...
///
/// Process-wide for the same reason [`rulogman_term::TerminalTheme`]'s custom
...
/// `.py` wins over the one rulogman ships even though both are custom. And a user
...
/// file they belong to, and so that the test over the definitions rulogman ships
...
"#,
    ...
    fn every_shipped_definition_compiles_with_nothing_dropped() {
        ...
        for (id, source) in SHIPPED {
            ...
            // a file rulogman ships must not need the forgiving path.
