...
impl PlatformWindow for MacWindow {
    ...
    /// RULOGMAN PATCH: see [`PlatformWindow::set_titlebar_transparent`].
    fn set_titlebar_transparent(
        ...
        executor
            .spawn(async move {
                ...
                // Going transparent the buttons move to wherever the app draws
                // its own title bar; going back they return to the frames
                // `move_traffic_light` captured before it first moved them,
                // which is what AppKit would have laid out.
                let mut this = state.as_ref().lock();
                if transparent {
                    this.move_traffic_light();
                } else {
                    this.restore_traffic_light();
                }
            })
            .detach();
    }
