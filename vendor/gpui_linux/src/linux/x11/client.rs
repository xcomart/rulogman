        ...
        match event {
            ...
            Event::ClientMessage(event) => {
                ...
                if atom == state.atoms.WM_DELETE_WINDOW {
                    // RULOGMAN PATCH: release the client borrow before *either*
                    // close callback runs, not just before `close`.
                    //
                    // Both re-enter the application, and anything the
                    // application does there that reaches the platform — a
                    // `quit`, a window opened or restyled — borrows this same
                    // RefCell and panics with "already borrowed". Upstream
                    // learnt half of that: `close` runs outside the borrow
                    // since the crate was split, which is what an app that
                    // quits from its window-closed observer, as this one does,
                    // needs. `should_close` did not move, so a
                    // `Window::on_should_close` handler is still standing on
                    // the same trapdoor; this app registers none today, and
                    // the patch is what keeps that from being a condition of
                    // closing a window.
                    //
                    // A WM_DELETE_WINDOW message is a WM_PROTOCOLS message,
                    // never an XdndEnter, so returning early skips nothing
                    // below.
                    drop(state);
